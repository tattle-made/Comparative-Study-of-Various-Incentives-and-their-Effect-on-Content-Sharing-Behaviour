...
const PositionedNews = styled.div`
  width: 100%;
  height: 100%;
  background-color: ${(props) => (props.ix % 2 === 0 ? "red" : "aqua")};
...
function FeedItem({ ix, item }) {
  ...
  return (
    <Child key={ix} ix={ix}>
      <Box margin={"medium"} pad={"medium"} border round>
        <Heading level={1}>{item.headline}</Heading>
        <Button plain label={"Read More"} onClick={() => setExpand(!expand)} />
        {expand ? <Paragraph size="xxlarge">{item.readMore}</Paragraph> : null}
      </Box>
    </Child>
  );
}
